context('Blauberg', () => {
  ...
  it('automatic_selection_pre_heater', () => {
    ...
    const pre_water_heater = ['not.exist', 'not.exist', 'exist'];
    var i = 0;
    cy.get('#supply_pre_heater option').each(($el) => {
      ...
      if (pre_heater[i] === 'exist') {
        cy.log('pre-heater water ' + pre_water_heater[i]);
        cy.get('#results-data')
          .contains('Pre-heater')
          .parents('div')
          .next('div')
          .find('#calculation_pre_heater_water_in')
          .should(pre_water_heater[i]);
      }
      i++;
    });
    cy.checkPdf();
...
//cooler
    it.only('automatic_selection_cooler', () => {
  
      cy.get('#select2-supply_cooler-container').click()
      var cooler = ['not.exist', 'exist', 'exist']
      const cooler_water = ['not.exist', 'not.exist', 'exist'];
      var i = 0;
      cy.get('#supply_cooler option').each(($el) => {
        cy.get($el).invoke('attr', 'value').then(val => {
          cy.get('#supply_cooler').select(val, { force: true })
          cy.log(val)
        })
          cy.get($el).invoke('text').then(text => {
            cy.log(text)
        cy.get('#select2-supply_cooler-container').contains(text, { matchCase: false })
      })
        cy.get('#automatic_calculate-submit').click({ force: true })
        cy.wait(3000);
        cy.checkTitle();
        cy.get('#results-data').contains('Cooling coil').should(cooler[i])
        if (cooler[i] === 'exist') {
          cy.log('cooler water ' + cooler_water[i]);
          cy.get('#results-data')
            .contains('Cooling coil')
            .parents('div')
            .next('div')
            .find('#calculation_summer_water_in')
            .should(cooler_water[i]);
        }
        i++;
      });
      cy.checkPdf();
...
//install type ceiling, vertical, horizontal
  ...
  it('automatic_selection_app_code', () => {
      cy.get('#automatic_calculate-submit').click({ force: true })
      cy.wait(1000);
      cy.checkTitle();
