...
//fans
  //supply
  cy.get('#supply_fan').find('h2').should('have.text','R3G400PA2703')
    ...
    //sound table
    cy.get('#supply_fan').find('#sound_table_value tbody').children('tr').should('have.length', 6).should("not.be.empty").should('be.visible')
    var i = 0
    var sound_value = [65,60,61,63,57,58,59,48,66]
    cy.get('#supply_fan').find('#sound_table_value tbody').children('tr').eq(0).find('td').each(($el) => {
      if ($el !== $el[0] || $el !== $el[0]) {

        cy.get($el).invoke('text').should('be.a', 'string').
          then(parseFloat).should('be.a', 'number').and('equal', sound_value[i]);
        i++
      }
    })
