context('Blauberg', () => {
  before( () => {
    ...
    beforeEach(() => {
      ...
      cy.visit(Url+'/heat_exchanger/crossflow')
  })

  it('calculate_button', () => {
    cy.get('#select2-model-container').click().type('Hp-EX4 250{enter}');
    //cy.get('#select2-model-result-cxae-Hp-EX4 250').click();
    ...
    cy.get('#calculation_winter_h_standard_airflow').should('be.visible').invoke('text').should('be.a', 'string').
    then(parseInt).should('be.a', 'number').and('equal', 250);
